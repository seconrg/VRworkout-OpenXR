...
func load_persistent_config(location):
	...
	if error == OK:
		...
		config_file.close()
		parameters = tmp
		print ("Config loaded as", location)
	else: 
		print ("Could not open config at", location )
		...
		while fname != "":
			if not dir.current_is_dir():
				...
				if fields and (fields[-1] == "ogg" or fields[-1] == "mp3" or fields[-1] == "import"):
					...
					var file_path = ""
					if path.find("res://") == 0:
						file_path = "res://" + full_path
					else:
						file_path = full_path
					song_dict[file_path] = get_song_name(full_path)
			fname = dir.get_next()
	# print(path, "cur song list", song_dict.keys())
