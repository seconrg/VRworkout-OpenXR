...
# on mobile devices, this is a private path and nt accessbile by other apps.
...
func create_database_entry(song):
	...
	var duration = 0
	var loaded = false
	print("name ", name, "duration ", duration, "file ", song, "loaded ", loaded)
...
func get_song_duration(songfile):
	...
	if song_database.has(songfile):
		print("Get duration about song named: ", songfile);
...
func intialize_song_database():
	...
	var songs = []
	# these songs are stored locally in the private storage system
	...
	# print("current local database info:")
	# for song_info in song_database:
	# 	print(song_info)
	# print("current song database info: ")
	# for song_info in song_database:
	# 	print(song_info)
