...
func initialize():
	...
	if arvr_ovr_mobile_interface:
		...
		if arvr_oculus_interface.initialize():
			...
			GameVariables.vr_mode = true
			get_viewport().arvr = true;
			Engine.target_fps = 120 # TODO: this is headset dependent (RiftS == 80)=> figure out how to get this info at runtime
			OS.vsync_enabled = false;
		...
		if arvr_open_vr_interface.initialize():
			...
			get_viewport().arvr = true;
			Engine.target_fps = 120 # TODO: this is headset dependent => figure out how to get this info at runtime
			OS.vsync_enabled = false;
			GameVariables.vr_mode = true;	
